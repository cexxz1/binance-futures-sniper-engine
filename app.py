...
def scan_and_update():
    ...
    for pos in open_positions:
        ...
        try:
            ...
            # Ağırlıklı piramitleme ve kilitli pozitif breakeven kalkanı (80/80/200 hyper compound)
            if direction == 'LONG':
                new_peak = max(peak, h)
                gain = (new_peak - entry) / entry
                init_m = margin / (1.0 + (pyr == 1) * 0.80 + (pyr == 2) * 1.60 + (pyr == 3) * 3.60)
                ...
                if gain >= 0.05:
                    sl = max(sl, new_peak * 0.935) # %6.5 Peak Trailing Stop (Winner)
