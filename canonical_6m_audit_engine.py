# C:\Users\depco\OneDrive\Desktop\mtf_signal_engine\canonical_6m_audit_engine.py
...
def run_6m_audit():
    ...
    for s in CHAMPIONS:
        # Fetch in 5 batches of 1000 (going backwards)
        all_raw = []
        end_t = None
        for _ in range(5):
    ...
    for t in all_times:
        ...
        lev = 12 if m in [3, 4, 7, 9] else 16
            ...
            if t in d['times']:
                ...
                if t > active_pos['entry_ts']:
                    ...
                    if dirn == 'LONG':
                        ...
                        if gain >= 0.05:
                            active_pos['sl'] = max(active_pos['sl'], peak * 0.92)

                        exit_now = (l <= active_pos['sl'])
                        exit_p = active_pos['sl'] if exit_now else p

                    else:
                        trough = min(active_pos['peak'], l)
                        gain = (entry - trough) / entry
                        if active_pos['pyr'] == 0 and gain >= 0.03:
                            add_m = init_m * 0.70
                            if bal >= add_m:
                        ...
                        if gain >= 0.05:
                            active_pos['sl'] = min(active_pos['sl'], trough * 1.08)
        ...
        if not active_pos and bal > 10.0 and t >= pause_until:
            ...
            for s in CHAMPIONS:
                ...
                # Safe base allocation: 60% for bal < $3000
                alloc_long = 0.60 if bal < 3000 else 0.90

                if long_ok:
                    m = min(slot_cap * alloc_long, max_safe_m)
...
if __name__ == '__main__':
    trades, b, v, mb = run_6m_audit()
    print(f"\n--- CANONICAL 6-MONTH AUDIT (180 DAYS) ---")
    print(f"Kasa: ${b:,.2f} | Vault: ${v:,.2f} | Toplam Net Servet: ${b+v:,.2f} | En Dip: ${mb:.2f} | Toplam İşlem: {len(trades)}")
