# C:\Users\depco\OneDrive\Desktop\mtf_signal_engine\run_1m_clean_audit.py
import sys, datetime, math
sys.path.append(r"C:\Users\depco\OneDrive\Desktop\mtf_signal_engine")
from canonical_6m_audit_engine import fetch_klines, CHAMPIONS
...
def run_1m_clean():
            ...
            if t in d['times']:
                ...
                if t > active_pos['entry_ts']:
                    ...
                    if exit_now:
                        ...
                        trades.append({
                            'open': datetime.datetime.fromtimestamp(active_pos['entry_ts']/1000, datetime.timezone.utc).strftime('%m-%d %H:%M'),
                            'close': dt.strftime('%m-%d %H:%M'),
                            'sym': s, 'dir': dirn, 'in': entry, 'out': exit_p,
                            'pnl': total_pnl, 'bal': bal
                        })
                        active_pos = None
    ...
    return trades, bal, min_bal

if __name__ == '__main__':
    tr, b, mb = run_1m_clean()
    print(f"1-AY KANONIK SONUC: Bakiye=${b:,.2f} | En Dip=${mb:.2f} | Islem={len(tr)}")
    for idx, t in enumerate(tr, 1):
        print(f"[{idx:02d}] {t['open']} -> {t['close']} | {t['sym']:<10} {t['dir']:<5} | In:${t['in']:<8.4f} Out:${t['out']:<8.4f} | PnL:${t['pnl']:>8.2f} | Kasa:${t['bal']:>8.2f}")
